...
if page == "Home":
    ...
    avg_amt_lkr = df['Amount'].mean() * 300  # Convert to LKR
    ...
    with col4:
        ...
        <div class="metric-card" style="border-top: 4px solid #34D399;">
            ...
            <div class="metric-value" style="color: #34D399;">Rs. {avg_amt_lkr:,.2f}</div>
        </div>
        """, unsafe_allow_html=True)
...
# -------------------------------------------------------------------------
    ...
    if model is None:
        ...
        # Set defaults based on template (converted to LKR)
        if template_choice == "Standard Legitimate Transaction":
            val_amount, val_time, val_v14, val_v17, val_v4, val_v11 = 13560.0, 45000, 0.50, 0.20, -0.10, 0.00
        elif template_choice == "High-Value Suspicious Purchase":
            val_amount, val_time, val_v14, val_v17, val_v4, val_v11 = 975000.0, 12000, -4.80, -5.20, 3.80, 2.90
        elif template_choice == "Rapid Micro-Payments":
            val_amount, val_time, val_v14, val_v17, val_v4, val_v11 = 2550.0, 3600, -3.20, -2.80, 2.50, 3.00
        else:
            val_amount, val_time, val_v14, val_v17, val_v4, val_v11 = 37350.0, 86400, 0.10, 0.05, -0.05, 0.02
        ...
        with col1:
            tx_amount = st.number_input("Transaction Amount (Rs.)", min_value=0.01, max_value=30000000.0, value=val_amount, step=1000.0)
        ...
        if st.button("🔍 Analyze Transaction", use_container_width=True, type="primary"):
            ...
            if tx_amount > 300000.0:
                attribution_items.append(f"**High Transaction Amount**: Rs. {tx_amount:,.2f} (unusual value)")
...
# -------------------------------------------------------------------------
    ...
    if model is None:
        ...
        if len(false_positives) > 0:
            ...
            st.markdown(f"- Average Amount: Rs. {fp_avg_amount * 300:,.2f}")
        ...
        if len(false_negatives) > 0:
            ...
            st.markdown(f"- Average Amount: Rs. {fn_avg_amount * 300:,.2f}")
